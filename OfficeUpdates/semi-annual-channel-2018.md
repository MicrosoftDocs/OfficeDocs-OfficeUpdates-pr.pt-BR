...
Estas notas sobre a versão fornecem informações sobre recursos, atualizações de segurança e outras atualizações não relacionadas à segurança incluídas nas atualizações do Canal Semestral do Office 365 ProPlus em 2018. 

> [!NOTE]
> - A seguir também são fornecidas informações sobre recursos, atualizações de segurança e outras atualizações não relacionadas à segurança do Visio Pro para Office 365 e do Project Online Desktop Client.
...
-   [CVE-2018-8163](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8163): Vulnerabilidade de Divulgação Não Autorizada de Informações do Microsoft Excel 
-   [CVE-2018-8246](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8246): Vulnerabilidade de Divulgação Não Autorizada de Informação do Microsoft Excel
...
-   Correção de um problema em que os arquivos temporários de recuperação automática criados durante a recuperação do documento não são limpos.
-   Correção de um problema em que o Excel falha ao abrir um arquivo do SharePoint Online.
-   Correção de um problema em que a Impressão Rápida de uma pasta de trabalho do Excel anexada a um email do Outlook pode não ser impressa.
-   Correção de um problema em que clicar em um hiperlink pode causar uma falha no Excel.
-   Correção de um problema em que o uso das funções de cubo pode causar uma falha no Excel.
-   Correção de um problema em que a impressão ou a visualização da impressão só imprime ou exibe uma parte da planilha, com o conteúdo truncado em uma segmentação na planilha.
-   Correção de um problema em que a tentativa de fazer uma nova conexão a um arquivo de texto em uma pasta de trabalho protegida causa a mensagem de erro "A pasta de trabalho está protegida e não pode ser alterada". Correção de um problema em que, se o idioma de edição for o japonês, chinês ou coreano, o Excel poderá congelar ao tentar escolher uma nova fonte na guia Página Inicial ou ao editar.
-   Correção de um problema em que as barras de rolagem sumiam ao abir uma pasta de trabalho quando o Excel estava minimizado.
...
-   **Configuração de recursos do LinkedIn:** Acesse Arquivo \> Opções \> Geral para controlar a exibição dos recursos do LinkedIn nos aplicativos do Office. [Saiba mais](https://support.office.com/article/dc81cc70-4d64-4755-9f1c-b9536e34d381)
-   **Modelos 3D:** Use o 3D para aumentar o impacto visual e criativo do seu e-mail. Insira facilmente um modelo 3D e gire-o 360 graus. [Saiba mais](https://support.office.com/article/ec5feb79-b0af-47f6-a885-151fcc88ac0a)
...
-   [CVE-2018-8310](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8310): Vulnerabilidade de Violação do Microsoft Office
...
-   Correção de um problema em que o Outlook falha ao resolver a exibição de nomes para endereços de email quando o usuário redige um email.
...
-   **Melhoria do Designer:** O Designer agora sugere ideias de design para linhas do tempo em uma lista com marcadores.
-   **Sons úteis melhoram a acessibilidade:** Ative as indicações de áudio para orientá-lo durante o trabalho. Para encontrar o recurso, vá para Arquivo \> Opções \> Facilidade de Acesso. Não é necessário usar suplementos. [Saiba mais](https://support.office.com/article/49fda9be-cce5-4c72-a87f-b11000197f5f)
-   **Locais de arquivos por conta:** ao abrir ou salvar um arquivo, a lista de locais é organizada pela conta associada a eles.
-   **Personalização de canetas:** Escolha um conjunto pessoal de canetas e marca-textos para escrita à tinta. O conjunto personalizado ficará disponível em todos os computadores Windows.
-   **Melhoria do Designer:** O Designer agora sugere ideias de design para os gráficos adicionados aos slides.
...
-   Correção de um problema causado quando uma tarefa era dividida com um recurso de custo fazendo com que o recurso de custo não fosse atualizado corretamente e o custo fosse perdido.
...
-   Correção de um problema onde salvar como XML poderia causar uma falha para os projetos mestres no Project Online ou no Project Server.
-   Correção de um problema em que, ao adicionar tarefas existentes à caixa de diálogo da Linha do Tempo, somente as tarefas da primeira tarefa resumo eram exibidas no Modo de Exibição da Linha do Tempo.
-   Correção de um problema em que o uso do menu suspenso Autofiltro em uma coluna de datas fazia com que todas as tarefas do projeto ficassem ocultas.
...
-   Correção de um problema em que, ao definir mais de uma linha de base em uma sessão, o valor MOD\_DATE era definido igualmente.
...
-   Correção de um problema em que adicionar usuários escolhendo "Chamada do Skype" em uma reunião causava um erro
-   Correção de um problema em que adicionar usuários escolhendo "Chamada do Skype" em uma reunião causava um erro.
...
-   **Criar um diagrama delineado do aplicativo móvel:** use um modelo para criar um diagrama delineado do aplicativo móvel. [Saiba mais](https://support.office.com/article/2d54dc55-f5c4-49a2-85da-d649eb7fc281)
...
-   Correção de um problema em que os números de página em algarismos romanos em letra minúscula são alterados de forma incorreta para maiúsculas.
-   Correção de um problema que causa a exibição de uma mensagem de memória insuficiente.
-   Correção de um problema em que o Word não abre em um computador rodando o Windows 7 sem a atualização para telemetria de diagnóstico e experiência do usuário instalada.
...
-   Correção de um problema em que, ao abrir um aplicativo, o usuário podia ver uma mensagem sobre a inicialização em modo de segurança e o aplicativo não abria.
...
-   Correção de um problema que causa mudanças no formato de arquivo .box, o que afeta a funcionalidade das versões mais antigas do Office instaladas no mesmo computador visto que os arquivos .box são compartilhados entre todas as versões de um aplicativo do Office no mesmo computador.
...
-   **Propriedade Nome do Rótulo:** melhora a acessibilidade associando um rótulo a um controle ou formulário.
-   **A edição de um item novo está mais acessível:** Use o atalho de teclado (Ctrl+E) rápido para editar um novo item em uma Caixa de Combinação ou em uma Caixa de Listagem.
-   **Conector Dynamics:** Importar dados ou vincular a dados armazenados no Microsoft Dynamics. [Saiba mais](https://support.office.com/article/636079c1-9fc3-4fca-8410-6596d62223da)
-   **Conector Salesforce:** Importar dados ou vincular a dados armazenados na Salesforce. [Saiba mais](https://support.office.com/article/7375ffb6-1d6a-46f1-bb0c-c6ac3c58f5a0)

### <a name="access-non-security-updates"></a>Access: Atualizações não relacionadas à segurança
-   Correção de um problema em que tentar selecionar o texto em uma caixa de texto ou caixa de combinação selecionava todo o texto em vez da seleção indicada.
-   Correção de um problema em que uma consulta não era executada se ela possuísse uma junção com uma chave primária de uma tabela vinculada do Microsoft Dynamics.
-   Correção de um problema em que as casas decimais não eram mostradas para valores em forma de moeda em uma tabela do Microsoft Dynamics.

### <a name="excel-feature-updates"></a>Excel: Atualizações de recursos
-   **Melhorias do recurso "Adicionar Coluna dos Exemplos":** Suporte para mais transformações de Data/Hora, Matemática e Índice de Coluna.
-   **Melhorias de desempenho:** O Excel abre pastas de trabalho complexas com várias planilhas mais rápido para que você possa processar fórmulas com intervalos maiores, filtrar muitas linhas ou copiar e colar mais rápido.
...
-   **Conector do Azure Data Lake Store:** Os usuários agora podem importar dados do Azure Data Lake Store.
-   **Melhorias do recurso "Adicionar coluna dos Exemplos":** Suporte para sugestões, mais operações de Data/Hora e outras transformações.
-   **Guia Dados**: Os botões da faixa de opções na guia Dados foram reorganizados em dois novos grupos: Obter & Transformar Dados e Consultas & Conexões.
-   **Compartilhar consultas**: Exporte qualquer definição de consulta em um arquivo ODC (Conexão do Banco de Dados do Office) e compartilhe entre pastas de trabalho ou com outras pessoas.
...
-   Correção de um problema em que o usuário não consegue fechar uma pasta de trabalho no modo de exibição protegido quando o nome do arquivo contém colchetes.
...
-   Correção de um problema em que, ao abrir uma pasta de trabalho clicando duas vezes nela, as fontes e os formatos das células de texto não são carregados ou duas pastas de trabalho idênticas são abertas para um único modelo.
...
-   Correção de um problema com a renderização onde cabeçalhos e linhas pretas aparecem devido à um driver gráfico com problema.
...
-   Correção de um problema em que colar uma Tabela Dinâmica em outra pasta de trabalho pode resultar em uma falha.
-   Correção de um problema em que, ao selecionar o comando Substituir e a caixa de diálogo Localizar e Substituir abrir, o foco da caixa de diálogo fica na guia Localizar em vez da guia Substituir.
-   Correção de um problema em que o Excel falha ao abrir o painel Atividade de uma pasta de trabalho aberta em um servidor do SharePoint anterior ao SharePoint Server 2016.
...
-   Correção de um problema no qual o Excel trava temporariamente ao expandir ou recolher uma Tabela Dinâmica e os cabeçalhos dela se movem para fora da tela.
...
-   Correção de um problema em que a atualização de dados não funciona ou o Excel falha durante o uso de dados de um servidor SQL Server Analysis Services e a localidade do Excel e do servidor SQL Server Analysis Services são diferentes.
...
-   Correção de um problema em que não é possível fazer alterações na planilha quando existe uma Tabela Dinâmica com campos na área de Filtragem e nenhum campo em outro local.

### <a name="outlook-feature-updates"></a>Outlook: Atualizações de recursos
-   **Melhorias de acessibilidade:** Facilitamos a leitura e a edição de texto, tabelas, listas e imagens no email durante o uso de um leitor de tela.
-   **Nova configuração de conta:** Configure novas contas com um novo assistente que requer menos etapas manuais.
-   **Caixa de diálogo Anexar para links:** Ao anexar um link usando o recurso "Anexar Arquivo" na faixa de opções, é possível escolher entre adicioná-lo como link ou como anexo. Caso prefira não exibir sempre esta caixa de diálogo, acesse Arquivo \> Opções \> Geral e especifique como deseja anexar os links em "Opções de Anexo".
-   **Suporte para anexos no local:** Os arquivos do SharePoint Server no local são exibidos como arquivos recentes em Mensagem \> Anexar Arquivo, os sites da equipe do OneDrive for Business e do SharePoint no local são exibidos em Anexar Arquivo \> Procurar Locais Web e os arquivos locais podem ser carregados para os sites do OneDrive for Business no local.
-   **Classificações de atividade para grupos:**  Um nível de classificação de atividade definido pelo administrador do locatário, como Confidencial, pode ser atribuído ao criar ou ao editar um grupo e esta classificação será exibida no cabeçalho do grupo.
...
-   Correção de um problema que causa falha intermitente ao abrir o Outlook.
...
-   Correção de um problema em que abrir determinados arquivos no Project Online faz com que o Project falhe.
-   Correção de um problema em que o Início Real pode ser limpo por engano ao definir o trabalho restante.
-   Correção de um problema onde a Atribuição da Data de Início Real pode mostrar dados diferentes dos que foram relatados pelo recurso por meio de status no Project Web App.
-   Correção de um problema em que o trabalho real pode ser reprogramado se a data de fim da tarefa for alterada.
...
-   Correção de um problema em que os dados da divisão ao longo do tempo para recursos de Orçamento não são copiados quando você salva de uma linha de base para outra.
...
-   Correção de um problema em que o Project falha se você tiver uma tabela cuja primeira coluna é o Nome da Tarefa, a coluna esteja bloqueada e uma tarefa seja clicada.
-   Correção de um problema em que você pode atribuir o mesmo recurso várias vezes à mesma tarefa por meio do modo de exibição de Uso de Tarefa.
...
-   Correção de um problema em que o recuo da tarefa de nível superior não sincroniza corretamente do Project para uma lista de tarefas do SharePoint.
-   Correção de um problema em que, se você importar uma tarefa, recurso ou informação sobre atribuição de uma pasta de trabalho do Excel, os valores no campo de trabalho podem ser ignorados.
-   Correção de um problema em que os valores de linha de base da divisão ao longo do tempo não correspondem aos valores iniciais ao salvar um projeto no formato de arquivo XML.
...
-   Correção de um problema para que seja mais rápido abrir arquivos do Project em um servidor de arquivos com alta latência.
...
-   [CVE-2017-8695](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-8695): Vulnerabilidade de Divulgação Não Autorizada de Informação do Componente Graphics
...
-   Correção de um problema em que as mensagens não lidas em salas de chat persistente são marcadas como lidas ao clicar nas guias de conversas de mensagens instantâneas.
-   Correção de um problema em que as notificações do sistema de mensagens de chat são exibidas com atraso de vários segundos.
...
-   Correção de um problema em que usuários de ingresso anônimo são impedidos de ingressar quando a federação está desabilitada e o ingresso anônimo não está bloqueado explicitamente pelo organizador da reunião.
-   Correção de um problema em que o número de convidados é exibido incorretamente para o organizador da reunião em reuniões que excedem o limite.
-   Correção de um problema em que o número de telefone não é exibido na notificação do sistema em chamadas PSTN de entrada.
-   Correção de um problema em que, ao usar a tecla Delete ao renomear um grupo da lista de contatos, o grupo inteiro é excluído.
-   Correção de um problema em que a notificação de compartilhamento nas mensagens de chat é ignorada antes da interrupção do compartilhamento.
-   Correção de um problema em que a tela de entrada fica em branco para alguns idiomas diferentes do inglês.
-   Correção de um problema em que os caracteres que não fazem parte da língua inglesa no chat e no histórico do chat ficam ilegíveis.
...
-   Adição da capacidade de ligar ou desligar a visualização do próprio vídeo para chamadas de vídeo P2P em VDIv2.
-   Correção de um problema em que números duplicados são exibidos para os contatos no menu suspenso da chamada.
...
-   Correção de um problema em que a opção Aparecer Offline não é exibida ao usar as políticas de cliente Habilitar Aparecer Offline e URL de Estado Personalizado.
-   Ampliação do botão Ingressar na Reunião para corrigir o truncamento em alguns idiomas.
-   Aumentar a importância das mensagens de Alta Prioridade no chat.
-   Adicionar os tipos de extensão de arquivo do Office e do Skype for Business como permitidos nas listas de bloqueios de transferência.
-   Correções de localização em convites para reunião do Outlook para o texto de rodapé da reunião definido em idioma diferente do inglês.
-   Correção de um problema em que é possível alterar os nomes dos remetentes em conversas de vários usuários.
-   Correção de um problema em que há uma falha na exibição da janela de conversa em branco até que o ingresso em uma reunião seja realizado com êxito.
-   Correção de um problema em que as informações do campo de departamento de um cartão de contato aparecem em branco nos resultados da pesquisa caso o campo de título esteja em branco.
...
-   Adição de uma chave de registro DWORD para corrigir um problema em que, quando o usuário entra no cliente por meio de uma rede externa executando o serviço de Descoberta Automática do Lync, o cliente redefine a chave do registro OAuthUsed para falsa. Para corrigir o problema, defina o valor de 1 para EnableRestoreOAuthUsedKeyWhenUsingCachedWebTicke em HKEY\_CURRENT\_USER\\Software\\Microsoft\\Office\\16.0\\Lync\\\<SipID\>.
...
-   [CVE-2018-0807](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0807): Vulnerabilidade de execução remota de código do Microsoft Word
-   [CVE-2018-0812](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0812): Vulnerabilidade de Corrupção de Memória do Microsoft Word
-   [Supervisão 170020](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/ADV170020): Atualização de Defesa Profunda do Microsoft Office
...
-   Correção de um problema com o zoom e o ajuste de escala em suplementos do Office em um ambiente de DPI dinâmico.
-   Correção de um problema em que o nó CurrentStatus do provedor de serviços de configuração (CSP) do Office retorna uma cadeia de caracteres vazia mesmo se o Office 365 ProPlus estiver instalado atualmente.
-   Correção de um problema que causa mudanças no formato de arquivo .box, o que afeta a funcionalidade das versões mais antigas do Office instaladas no mesmo computador visto que os arquivos .box são compartilhados entre todas as versões de um aplicativo do Office no mesmo computador.
-   Correção de um problema em que, em determinadas circunstâncias ao usar a ativação de computador compartilhado, uma mensagem de erro é exibida informando que o aplicativo encontrou um erro que o impede de funcionar corretamente e perguntando se o usuário deseja realizar um reparo.
...
-   Correção de um problema em que alguns módulos de VBA com nomes com caracteres byte duplo não podem ser abertos.
-   Correção de um problema que impede que a caixa de diálogo Novidades seja exibida.
-   Correção de um problema em que, ao clicar na guia Desenhar, ocorre uma falha no aplicativo para alguns usuários.
...
-   Correção de um problema relacionado à maneira pela qual alguns arquivos de programas são assinados fazendo com que programas antivírus sinalizem esses arquivos, além de problemas para proteger ou acessar dados na Proteção de Informações do Windows(WIP).
...
-   Correção de um problema em que os comandos estão ausentes das caixas de diálogo de personalização da Faixa de Opções ou da Barra de Ferramentas de Acesso Rápido.
